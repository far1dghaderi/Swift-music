...
function parseAudioTime(number: number): string {
  return (number / 60).toFixed(2);
}
function getSongsSrc(parent: Element): string[] {
  let srcs: string[] = [];

  parent.childNodes.forEach((child) => {
    if (child instanceof Element) {
      if (
        child.classList.contains("player-queue-song") &&
...
function changePLayIcons(selectedElement: Element) {
  selectedElement.classList.replace(
    "bi-play-circle-fill",
    "bi-pause-circle-fill"
  );
  if (controlboxPlayBtn) {
    controlboxPlayBtn.classList.replace(
      "bi-play-circle-fill",
      "bi-pause-circle-fill"
    );
  }
}
let queueSongs: string[] = [];
...
if (progressbar && progressbar instanceof HTMLInputElement) {
  progressbar.addEventListener("input", (): void => {
